...
BOOL GetRawDeviceName(LPCWSTR pDeviceName, LPWSTR pRawDeviceNameBuffer, size_t cchBuffer)
{
	...
	if (hr == STRSAFE_E_INSUFFICIENT_BUFFER) {
		logw(L"Failed because of STRSAFE_E_INSUFFICIENT_BUFFER for DeviceName <%s>. Number of chars is <%Iu>\n", pDeviceName, raw.length());
	}

	if (hr == STRSAFE_E_INVALID_PARAMETER) {
		logw(L"Failed because of STRSAFE_E_INVALID_PARAMETER for DeviceName <%s>\n", pDeviceName);
	}
...
BOOL SendReleaseIRP(LPCWSTR	DeviceName)
{
	...
	{
		logw(L"Not able to get the RawDeviceName for DeviceName:%ws\n", DeviceName);
		return FALSE;
	}
